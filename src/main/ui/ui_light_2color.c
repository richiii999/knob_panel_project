...
static void light_2color_layer_timer_cb(lv_timer_t *tmr){
    ...
    if (is_time_out(&time_20ms)) {
        if ((light_set_conf.light_pwm ^ light_xor.light_pwm) || (light_set_conf.light_cck ^ light_xor.light_cck)) {
            ...
            if (LIGHT_CCK_COOL == light_xor.light_cck) { RGB_color = (0xFF * light_xor.light_pwm / 100) << 16 | (0xFF * light_xor.light_pwm / 100) << 8 | (0xFF * light_xor.light_pwm / 100) << 0; } 
            else {                                       RGB_color = (0xFF * light_xor.light_pwm / 100) << 16 | (0xFF * light_xor.light_pwm / 100) << 8 | (0x33 * light_xor.light_pwm / 100) << 0; }
            ...
            if (light_set_conf.light_pwm) { lv_label_set_text_fmt(label_pwm_set, "%d%%", light_set_conf.light_pwm); } 
            else { lv_label_set_text(label_pwm_set, "--"); }
...
void lightTask(void* paramsList){ // params = [event bits, uint8_t lightLevel, uint8_t cck_set]
    ...
    switch (*lightLevel) { // Switch the light level, set the LED to the corresponding level.
        case 100: lv_obj_clear_flag(img_light_pwm_100, LV_OBJ_FLAG_HIDDEN); lv_img_set_src(img_light_pwm_100, light_image.img_pwm_100[*cck_set]); break;
        case 75 : lv_obj_clear_flag(img_light_pwm_75,  LV_OBJ_FLAG_HIDDEN); lv_img_set_src(img_light_pwm_75,  light_image.img_pwm_75[ *cck_set]); break;
        case 50 : lv_obj_clear_flag(img_light_pwm_50,  LV_OBJ_FLAG_HIDDEN); lv_img_set_src(img_light_pwm_50,  light_image.img_pwm_50[ *cck_set]); break;
        case 25 : lv_obj_clear_flag(img_light_pwm_25,  LV_OBJ_FLAG_HIDDEN); lv_img_set_src(img_light_pwm_25,  light_image.img_pwm_25[ *cck_set]); break;
        case 0  : lv_obj_clear_flag(img_light_pwm_0,   LV_OBJ_FLAG_HIDDEN); lv_img_set_src(img_light_bg,     &light_close_bg);                    break;
        default: break;
    }

    *eventBits = (*eventBits | (1 << 0)); // set the 0 bit (not atomic, couldnt get atomic function to work)
    vTaskDelete(NULL); // Self-deletion of task
}

void audioTask(void* paramsList){ // params = [event bts, uint8_t lightLevel] 
EventBits_t* eventBits = ((EventBits_t **)paramsList)[0];
uint8_t* lightLevel = ((uint8_t **)paramsList)[1];

    switch (*lightLevel) { // Switch the light level, play the corresponding sound
        case 100: audio_handle_info( 0 ); break; // (0-4) -> sound files selected audio.c/h
        case 75 : audio_handle_info( 1 ); break;
        case 50 : audio_handle_info( 2 ); break;
        case 25 : audio_handle_info( 3 ); break;
        case 0  : audio_handle_info( 4 ); break;
        default: break; 
    }

    *eventBits = (*eventBits | (1 << 1)); // set the 1 bit (not atomic, couldnt get atomic function to work)
    vTaskDelete(NULL); // Self-deletion of task
}
